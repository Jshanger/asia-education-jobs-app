...
// Parse EXTRA_RSS env (accepts JSON array or newline/comma/semicolon list)
...
// Keywords to filter by (if STRICT_ASIA_ONLY = 1)
const ASIA_WORDS = [
  // NEA & SEA countries and major cities
...
const ASIA_RE = new RegExp(
  `\\b(${ASIA_WORDS.map(w => w.replace(/[.*+?^${}()|[\\]\\\\]/g,"\\$&")).join("|")})\\b`, "i"
);

// Build feed URLs from Jobs.ac.uk, Times Higher Education, WISHlistjobs, Guardian, ChinaUniversityJobs and PIE
...
const RSS_SOURCES = [
  // Jobs.ac.uk: functional roles and country keywords
  ...
  // Times Higher Education (THEunijobs)
  ...
  // Guardian jobs (broad education + Asia)
...
// ---- Optional HTML fallback for PIE Jobs list page ----
...
// Normalize URL to de-dupe (strip common tracking params)
  ...
  try {
    ...
    for (const s of RSS_SOURCES) {
      try {
        ...
        // Skip if it's clearly HTML (not RSS/Atom)
    ...
    // Optional Asia filter (server side)
    if (STRICT_ASIA_ONLY) {
    ...
    merged = merged.slice(0, 2000); // Keep plenty of jobs
